...
class Pipeline {
  ...
  /// Optional explicit list of files / directories / globs to scan. When
  /// empty, the whole `lib/` directory is walked. Paths may be absolute or
  ...
  PipelineSummary scanOnly() {
    final stack = StackDetector.resolve(config, projectRoot);
    final scanRoot = p.join(projectRoot, 'lib');
    if (includePaths.isEmpty && !Directory(scanRoot).existsSync()) {
      return PipelineSummary(
        filesScanned: 0,
        totalCandidates: 0,
